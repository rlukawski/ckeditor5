...
describe( 'WordCount', () => {
	...
	describe( 'multi-root editor integration', () => {
		...
		it( 'should sum characters of each root', () => {
			setModelData( model, '<paragraph>foo</paragraph>', { rootName: 'foo' } );
			setModelData( model, '<paragraph>bar</paragraph>', { rootName: 'bar' } );

			expect( wordCountPlugin._getCharacters() ).to.be.equal( 6 );
		} );
		...
		it( 'should sum words of each root', () => {
			...
			expect( wordCountPlugin._getWords() ).to.be.equal( 2 );
		} );
	} );
} );
